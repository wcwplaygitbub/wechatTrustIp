...
- **Web 控制台**：`/console` 提供状态面板、日志查看、配置管理、密码修改等功能
...
| 页面 | 路径 | 说明 |
|------|------|------|
| 状态面板 | `/console/` | 显示当前公网 IP、已注册 IP、Cookie 状态，下次检测时间 |
| 日志 | `/console/logs` | 查看最近 200 条运行日志，扫码相关日志内嵌二维码 |
| 配置 | `/console/config` | 修改 IP 检测周期、应用 ID，支持热更新 |
...
1. **IP 检测**：通过多个 IP 查询服务获取当前公网 IP，与本地保存的 IP 对比
2. **IP 变化**：启动 Playwright 浏览器，用已有 Cookie 访问企业微信管理后台
3. **Cookie 失效**：打开登录页获取新二维码，推送到企业微信群，扫码后自动保存新 Cookie
4. **Session 保护**：保活操作直接访问后台首页（`/wework_admin/frame`），不经过登录页，避免淘汰客户端会话
